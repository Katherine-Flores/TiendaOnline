...
const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
        ...
        const extencionesPermitidas = ['.jpg'];
        if(!extencionesPermitidas.includes(extension)){
            return cb(new Error('Solo se permiten archivos con extensión .jpg'));
        }
        cb(null, true);
    }
});
...
const getProductos = async (req, res) => {
    try {
        ...
        // Convertir la foto de binario a Base64
        const productos = results.map(producto => {
            if (producto.foto) {
                producto.foto = `data:image/jpg;base64,${Buffer.from(producto.foto).toString('base64')}`;
            } else {
                producto.foto = null; // Si no hay foto se queda en null
            }
            return producto;
        });
        res.json(productos);
